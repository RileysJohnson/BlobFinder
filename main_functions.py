...
def BatchHessianBlobs(images_dict, params=None):
    ...
    for i, (image_name, im) in enumerate(images_dict.items()):
        ...
        # Validate measurement waves exist
        if not all([heights, avg_heights, areas, volumes]):
            print(f"   ERROR: Missing measurement waves for {image_name}")
            continue
        ...
        if len(heights.data) > 0:
            # FIXED: Proper wave concatenation with robust error handling
            try:
                ...
                # Progress reporting (Igor Pro style)
...
def SaveBatchResults(batch_results, output_path="", save_format="igor"):
    ...
    if save_format == "igor" or save_format == "txt":
        ...
        # HDF5 format for scientific data storage
        try:
            import h5py
            
            hdf5_file = os.path.join(output_path, f"{series_name}_Complete_{timestamp}.hdf5")
            
            with h5py.File(hdf5_file, 'w') as f:
                # Create groups for organization (Igor Pro style)
                params_group = f.create_group('Parameters')
                measurements_group = f.create_group('Measurements')
                images_group = f.create_group('ImageResults')
                ...
                # Save concatenated measurements
                measurements_group.create_dataset('AllHeights', data=batch_results['AllHeights'].data)
                measurements_group.create_dataset('AllVolumes', data=batch_results['AllVolumes'].data)
                measurements_group.create_dataset('AllAreas', data=batch_results['AllAreas'].data)
                measurements_group.create_dataset('AllAvgHeights', data=batch_results['AllAvgHeights'].data)
                
                # Save individual image results
                for image_name, results in batch_results['image_results'].items():
                    img_group = images_group.create_group(image_name.replace('.', '_'))
                    img_group.create_dataset('Heights', data=results['Heights'].data)
                    img_group.create_dataset('Areas', data=results['Areas'].data)
                    img_group.create_dataset('Volumes', data=results['Volumes'].data)
                    img_group.create_dataset('AvgHeights', data=results['AvgHeights'].data)
                    img_group.create_dataset('COM', data=results['COM'].data)
                    img_group.create_dataset('Info', data=results['info'].data)
                    
                # Metadata
                f.attrs['total_images'] = batch_results['numImages']
                f.attrs['total_particles'] = batch_results['numParticles']
